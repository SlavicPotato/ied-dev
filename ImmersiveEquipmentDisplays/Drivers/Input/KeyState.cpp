...
namespace IED
{
	...
	void KeyState::SetKeyState(KeyEventType a_ev, std::uint32_t a_sc, UINT a_vk)
	{
		SetKeyState(a_ev, a_vk);
		SetLRKeyState(a_ev, a_sc, a_vk);
	}

	void KeyState::SetKeyState(KeyEventType a_ev, UINT a_key)
	{
		if (a_ev == KeyEventType::KeyUp)
		{
	...
	void KeyState::SetLRKeyState(KeyEventType a_ev, std::uint32_t a_sc, UINT a_vk)
	{
		switch (a_vk)
		{
		case VK_MENU:
