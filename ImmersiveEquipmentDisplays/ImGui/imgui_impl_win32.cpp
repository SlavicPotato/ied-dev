...
// dear imgui: Platform Binding for Windows (standard windows API for 32 and 64
// bits applications) This needs to be used along with a Renderer (e.g.
// DirectX11, OpenGL3, Vulkan..)

// Implemented features:
//  [X] Platform: Clipboard support (for Win32 this is actually part of core
//  dear imgui) [X] Platform: Mouse cursor shape and visibility. Disable with
//  'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'. [X] Platform:
//  Keyboard arrays indexed using VK_* Virtual Key Codes, e.g.
//  ImGui::IsKeyPressed(VK_SPACE). [X] Platform: Gamepad support. Enabled with
...
// Using XInput library for gamepad (with recent Windows SDK this may leads to
// executables which won't run on Windows 7)
...
//  2020-03-03: Inputs: Calling AddInputCharacterUTF16() to support surrogate
//  pairs leading to codepoint >= 0x10000 (for more complete CJK inputs)
//  2020-02-17: Added ImGui_ImplWin32_EnableDpiAwareness(),
...
void ImGui_ImplWin32_Shutdown() { g_hWnd = (HWND)0; }
...
static void ImGui_ImplWin32_UpdateGamepads()
{
	...
	if (g_WantUpdateHasGamepad)
	{
		...
		g_HasGamepad =
			(XInputGetCapabilities(0, XINPUT_FLAG_GAMEPAD, &caps) == ERROR_SUCCESS);
...
// When implementing your own back-end, you can read the io.WantCaptureMouse,
...
// (GetCapture/SetCapture/ReleaseCapture) to be able to read mouse coordinates
...
IMGUI_IMPL_API void ImGui_DInput_Handler(uint32_t code, bool down)
{
