...
namespace IED
{
	...
	bool ConfigINI::Load(const std::string& a_path)
	{
		...
		m_odbNativeLoader      = false;  //reader.GetBoolValue(SECT_OBJECTDB, "UseNativeModelLoader", false);
		m_odbBackgroundLoading = reader.GetBoolValue(SECT_OBJECTDB, "BackgroundLoading", false);

		m_scNativeLoader = false; //reader.GetBoolValue(SECT_SKELETON_CACHE, "UseNativeModelLoader", false);
