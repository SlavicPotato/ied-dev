...
namespace IED
{
	namespace Papyrus
	{
		namespace Custom
		{
			...
			[[nodiscard]] constexpr Data::ConfigSex GetSex(bool a_female) noexcept
			{
				return a_female ? Data::ConfigSex::Female : Data::ConfigSex::Male;
			}

			template <class T>
			[[nodiscard]] constexpr auto GetConfigClass() noexcept
			{
				Data::ConfigClass cl;
			...
			[[nodiscard]] Data::NodeDescriptor GetOrCreateNodeDescriptor(const BSFixedString& a_node);

			[[nodiscard]] Data::configMapCustom_t& GetConfigMap(Data::ConfigClass a_class) noexcept;

			[[nodiscard]] Data::configCustomEntry_t* LookupConfig(
				Game::FormID             a_target,
				Data::ConfigClass        a_class,
				const stl::fixed_string& a_key,
				const stl::fixed_string& a_name);

			[[nodiscard]] std::optional<NiPoint3> GetNiPoint3(VMArray<float>& a_in);
